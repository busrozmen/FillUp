...
Context context2;
    ...
    public CircleMenuView(@NonNull Context context, AttributeSet attrs) {
    ...
    private void initMenu(final int menuButtonColor) {
    ...
    private void initButtons(@NonNull Context context, @NonNull List<Integer> icons, @NonNull List<Integer> colors) {
        ...
        String ActiveUserEmail = SP.getString("ActiveUserEmail", null);
        final int buttonsCount = Math.min(icons.size(), colors.size());
        if (ActiveUserEmail == null) {
            Toast.makeText(context, "Firstly, you must login!", Toast.LENGTH_LONG).show();
        }
        for (int i = 0; i < buttonsCount; i++) {
            ...
            if (i == 0 || i==4){
                ...
                if (ActiveUserEmail == null){
                    ...
                    button.setAlpha(DEFAULT_CLOSE_ICON_ALPHA);
                }
                else{
                    button.setClickable(true);
                    button.setBackgroundTintList(ColorStateList.valueOf(colors.get(i)));
    ...
    private Animator getOpenMenuAnimation() {
        final ObjectAnimator alphaAnimation = ObjectAnimator.ofFloat(mMenuButton, "alpha", DEFAULT_CLOSE_ICON_ALPHA);
            mMenuButton.setX(460);
            mMenuButton.setY(780);
    ...
    private Animator getCloseMenuAnimation() {
        ...
        set1.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mMenuButton.setImageResource(mIconMenu);
                SharedPreferences SP = context2.getSharedPreferences("MYPREFS", MODE_PRIVATE);
                String pageName = SP.getString("PageName", null);

                if(pageName.equals("MainActivity")){
                  mMenuButton.setX(460);
                    mMenuButton.setY(780);
                }
                else{
